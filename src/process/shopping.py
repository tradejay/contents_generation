...
class ShoppingPromptTool():
    ...
    def __call__(self, child_category, trend_type):
        ...
        return self.get_data(**param_ids), self.get_prompt()

    def save_image(self, save_dir="/Downloads", drop=True):
        for _, row in self.data.iterrows():
            save_img(row['imageUrl'], [get_today(), self.type_id, self.child_id, row["rank"]], save_dir)
        if drop:
    ...
    def get_fullprompt(self):
        ...
        user_content = "오늘 {} {} 요약해서 소개해줘".format(self.child_name, self.trend_type)
